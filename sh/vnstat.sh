...
# by @wim66
# v4 6-April-2024
# Adapted for conky-nextgen: interface passed as $1 (e.g. from
# conky_active_iface() in lua/hardware/network.lua) instead of a
# settings.lua that doesn't exist in this project; falls back to
...
output="../tmp/vnstat.txt"

# Step 1: interface passed in explicitly (preferred — keeps this in
# sync with whatever the network widget considers "active")
netw="$1"
...
if [ -z "$netw" ] || ! echo "$vnstat_ifaces" | grep -qx "$netw"; then
    [ -n "$netw" ] && echo "Interface '$netw' is not monitored by vnstat."
